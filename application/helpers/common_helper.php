...
{
	$url = "http://mysmsshop.in/V2/http-api.php?apikey=J1yIiWlrqdGCMj4w&senderid=RSHENT&number=$contact_no&message=" . urlencode($message_content) . "&format=json";
	$res = curl_init();
...
function getPartnerId($token)
{
	...
	$get = $ci->db->select()
		->from('partners')
		->where("id = '" . $token['data']->id . "' AND status = '1' AND unique_hash = '" . $token['data']->unique_hash . "'")
		->get();
	if ($get->num_rows() > 0) {
...
function orderIdGenerateUser2()
{
	$number = 'TXN' . date('ydmhis');
	if (checkOrderIdExistUser2($number)) {
		return	orderIdGenerateUser2();
	} else {
		return $number;
	}
}

function checkOrderIdExistUser2($number)
{
	$ci = &get_instance();
	$get = $ci->db->select()
		->from('rental_book_product')
		->where("transaction_id = '$number'")
		->get();
	if ($get->num_rows() > 0) {
		return true;
...
function documentUpload($imageName, $path)
{
	if (!file_exists($path)) {
		mkdir($path, 0777, true);
	}
